...
from diffmat.fracture.utilities import grid_spec, voxelise_particles_periodic
from diffmat.fracture.solver import elastodamage_phasefield_solve
from diffmat.fracture.rvegen import generate_particles_periodic
...
grid = grid_spec(box_size[0], box_size[1], box_size[2], spacing[0], spacing[1], spacing[2])
...
lmbda0 = 0.5 * (lmbda_grid.max() + lmbda_grid.min())
mu0 = 0.5 * (mu_grid.max() + mu_grid.min())
epsilon, sigma = lippmann_schwinger(
    compute_sigma_isotropic,
    {'lambda':lmbda_grid, 'mu':mu_grid},
    Emean_steps[0],
